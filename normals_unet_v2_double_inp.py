...
if __name__ == "__main__":
    ...
    wandb.init(
        ...
        notes="normals unet double input two vectors (up + right, down left len = 1)",
        config=wandb_config,
        mode="online"
    )
    name = "normals_unet_double_cross_" + datetime.now().strftime('%Y%m%d_%H%M%S') 
    wandb.run.name = name 
    device = "cuda:0"
    tl, vl, test = get_loaders_param(prep_cross_stacked)
    model, optimizer, scheduler = get_model_and_optimizer(device, in_ch=6)
