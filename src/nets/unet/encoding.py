...
class Encoder(nn.Module):
    ...
    def forward(self, x):
        skip_connections = []
        for n, encoding_block in enumerate(self.encoding_blocks):
            x, skip_connnection = encoding_block(x, step = 1)
...
class EncodingBlock(nn.Module):
    def __init__(
        ...
        if step:
            self.conv1_2 = ConvolutionalBlock(
                dimensions,
                in_channels,
                out_channels_first,
                normalization=normalization,
        ...
        if step:
            self.conv2_2 = ConvolutionalBlock(
                dimensions,
                out_channels_first,
                out_channels_second,
                normalization=self.normalization,
    ...
    def forward(self, x, step=None):
        if self.residual:
            ...
            if  step:
                orig_x = x
                x = self.conv1(orig_x[:,0:3, :,:])
                x = self.conv2(x)
                x_t = self.conv1_2(orig_x[:, 3:6, :,:])
                x_t = self.conv2_2(x_t)
            else:
                x = self.conv1(x)
                x = self.conv2(x)
