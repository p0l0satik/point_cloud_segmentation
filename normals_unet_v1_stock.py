...
if __name__ == "__main__":
    ...
    wandb.init(
        ...
        notes="unet normals stock 2 layers deep",
        config=wandb_config,
        mode="online"
    )
    name = "unet_normals_stock_2_" + datetime.now().strftime('%Y%m%d_%H%M%S') 
    wandb.run.name = name 
    device = "cuda:0"
    tl, vl, test = get_loaders_param(prep_stock)
    model, optimizer, scheduler = get_model_and_optimizer(device, num_encoding_blocks=2)
    starter, ender = torch.cuda.Event(enable_timing=True), torch.cuda.Event(enable_timing=True)
    starter.record()
